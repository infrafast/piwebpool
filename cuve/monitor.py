...
#endTime = str(now)
MAX_values = rrdtool.fetch(database_file, 'MAX','-s', 'end-12h', '-e', 'now')
niveau_maxi = (max(MAX_values[2])[0])
MIN_values = rrdtool.fetch(database_file, 'MIN','-s', 'end-12h', '-e', 'now')
niveau_mini = (min(MIN_values)[0])[0]

ratio30s = str(int(round(niveau_maxi / niveau_mini,2)-1)*100)+"%")
