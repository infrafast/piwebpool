...
if (!$result) {
    ...
    if (!$result) {
        ...
        if (!$result) {
            ...
            if (!weburl($materials,$pins,"http://admin:Quintal74604@domoticz.infrafast.com/json.htm?type=command&param=udevice&idx=27&nvalue=0&svalue=%ph")) $state.="{api update ph failed}";
            ...
            if (!weburl($materials,$pins,"http://admin:Quintal74604@domoticz.infrafast.com/json.htm?type=command&param=switchlight&idx=31&switchcmd=%t1")) $state.="{api update traitement1 failed}";            
        }
    }
}
