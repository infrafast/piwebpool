...
//increase
generate(24,7.3,7.3,300,300,$link,$materialsColumn,$materials,$pins);
//decrease
    ...
    for ($x = 0; $x <= $period; $x++) {
        ...
        if (!$result) {
            ...
            if (!$result) {
                ...
                $phValue=$phStep*$x+($phv1+(rand(0,5)<2?0:rand(-$phValue*0.05,$phValue*0.05)));
                $orpValue=$orpStep*$x+($orpv1+(rand(0,5)<2?0:intval(rand(-$orpValue*0.01,$orpValue*0.05))));
